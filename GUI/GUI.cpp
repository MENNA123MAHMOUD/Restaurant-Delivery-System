...
void GUI::PrintMessage(string msg1, string msg2,string msg3,string msg4) const	//Prints a message on status bar
{
	ClearStatusBar();	//First clear the status bar
	
	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD , BY_NAME, "Arial");   
	pWind->DrawString(10, WindHeight - (int) (StatusBarHeight/1.1), msg1); // You may need to change these coordinates later 
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.2), msg2); // to be able to write multi-line
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.3), msg3);
	pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.4), msg4);
}


void GUI::printdynamic(string msg, int x, double y)
{
	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
...
void GUI::DrawOrders() const
{
	...
	for(int i=0; i<OrderCount; i++)
	{
		Order* o1 = OrdListForDrawing[i];
		int orderRegion = o1->GetRegion();
...
{
	if ((ptr != nullptr) && OrderCount < MaxPossibleOrdCnt) 
